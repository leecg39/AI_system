  ...
  if (isLoading) {
    return (
      <div className="space-y-3 py-8" data-testid="task-request-loading">
        <h1 className="text-3xl font-black text-foreground">작업 요청</h1>
        <p className="text-muted-foreground">팀 설정과 에이전트 구성을 불러오는 중...</p>
      </div>
    );
  }
    ...
    return (
      <div className="space-y-4 py-8" data-testid="task-request-error">
        ...
        <div className="border-[3px] border-red-500 bg-red-50 p-4 text-red-700 shadow-[4px_4px_0_0_rgba(0,0,0,1)]">
          <p className="font-bold">팀 정보를 찾을 수 없습니다.</p>
  ...
  return (
    <div className="space-y-6 py-4" data-testid="task-request-page">
      <header className="space-y-2">
        <h1 className="text-3xl font-black text-foreground">작업 요청</h1>
        <p className="text-muted-foreground">{team.name} 팀의 에이전트 {agents.length}명에게 작업을 요청합니다.</p>
      </header>
      ...
      >
        <h2 className="text-sm font-black text-foreground">참여 에이전트</h2>
        {agents.length === 0 ? (
          <p className="mt-2 text-sm text-muted-foreground">연결된 에이전트가 없습니다.</p>
        ) : (
          <ul className="mt-2 grid gap-2 sm:grid-cols-2">
            {agents.map((agent) => (
              <li key={agent.id} className="border-[2px] border-foreground px-3 py-2 text-sm shadow-[2px_2px_0_0_rgba(0,0,0,1)]">
        ...
        {STEP_ITEMS.map((item) => {
          ...
          return (
            <li key={item.id}>
              <button
                type="button"
                className={`w-full border-[3px] px-4 py-3 text-left text-sm font-bold shadow-[3px_3px_0_0_rgba(0,0,0,1)] ${
                  isCurrent
              ...
              >
                <span className={`mr-2 inline-flex h-6 w-6 items-center justify-center rounded-full border text-xs font-black ${isDone ? "bg-green-600 text-white" : ""}`}>
                  {isDone ? "✓" : item.id}
                </span>
                {item.label}
              </button>
      ...
      {step === 1 ? (
        <section className="space-y-6 border-[3px] border-foreground bg-white p-6 shadow-[4px_4px_0_0_rgba(0,0,0,1)]" aria-label="옵션 선택">
          <div>
            <h2 className="text-lg font-black text-foreground">1단계: 옵션 선택</h2>
            <p className="mt-1 text-sm text-muted-foreground">작업 유형과 원하는 결과 포맷을 선택하세요.</p>
          </div>

          <fieldset className="space-y-3">
            <legend className="text-sm font-bold text-foreground">작업 유형</legend>
            <div className="grid gap-3 md:grid-cols-3">
              {preset.taskTypes.map((option) => (
                <label
                  key={option.value}
                  className={`cursor-pointer border-[3px] p-4 shadow-[3px_3px_0_0_rgba(0,0,0,1)] ${
                    taskType === option.value
                      ? "border-blue-600 bg-blue-50"
                      : "border-foreground bg-white hover:shadow-[1px_1px_0_0_rgba(0,0,0,1)] hover:translate-x-[2px] hover:translate-y-[2px] transition-all"
                  }`}
                >
                  <input
                    type="radio"
          ...
          <fieldset className="space-y-3">
            <legend className="text-sm font-bold text-foreground">결과 포맷</legend>
            <div className="grid gap-2 sm:grid-cols-2">
              {preset.outputs.map((output) => (
                <label
                  key={output.value}
                  className="flex items-center gap-2 border-[2px] border-foreground px-3 py-2 text-sm font-medium"
                >
                  <input
                    type="checkbox"
          ...
          <div className="space-y-2">
            <label htmlFor="execution-mode" className="text-sm font-bold text-foreground">
              실행 모드
            </label>
            <select
              id="execution-mode"
              className="h-10 w-full border-[2px] border-foreground bg-white px-3 text-sm"
              value={executionMode}
      ...
      {step === 2 ? (
        <section className="space-y-6 border-[3px] border-foreground bg-white p-6 shadow-[4px_4px_0_0_rgba(0,0,0,1)]" aria-label="입력">
          <div>
            <h2 className="text-lg font-black text-foreground">2단계: 입력</h2>
            <p className="mt-1 text-sm text-muted-foreground">URL, 텍스트, 파일 중 하나 이상을 제공하세요.</p>
          </div>

          <div className="space-y-2">
            <label htmlFor="source-url" className="text-sm font-bold text-foreground">
              소스 URL
            </label>
            <Input
              id="source-url"
          ...
          <div className="space-y-2">
            <label htmlFor="source-text" className="text-sm font-bold text-foreground">
              입력 텍스트
            </label>
            <textarea
              id="source-text"
              className="min-h-32 w-full border-[2px] border-foreground px-3 py-2 text-sm"
              value={sourceText}
          ...
          <div className="space-y-2">
            <label htmlFor="source-file" className="text-sm font-bold text-foreground">
              파일 업로드
            </label>
            <Input id="source-file" type="file" accept=".pdf,.txt,.md,.doc,.docx" onChange={handleFileChange} className="border-[2px] border-foreground" />
            {uploadedFile ? (
          ...
          <div className="space-y-2">
            ...
            <textarea
              id="instructions"
              className="min-h-24 w-full border-[2px] border-foreground px-3 py-2 text-sm"
      ...
      {step === 3 ? (
        <section className="space-y-6 border-[3px] border-foreground bg-white p-6 shadow-[4px_4px_0_0_rgba(0,0,0,1)]" aria-label="확인">
          <div>
            <h2 className="text-lg font-black text-foreground">3단계: 실행 전 확인</h2>
            <p className="mt-1 text-sm text-muted-foreground">선택한 옵션과 입력 데이터를 확인한 뒤 작업을 실행하세요.</p>
          </div>

          <dl className="grid gap-2 text-sm">
            <div className="flex justify-between gap-4 border-b-[2px] border-dashed border-foreground/20 pb-2">
              <dt className="font-bold text-foreground">팀</dt>
              <dd>{team.name}</dd>
            </div>
            <div className="flex justify-between gap-4 border-b-[2px] border-dashed border-foreground/20 pb-2">
              <dt className="font-bold text-foreground">작업 유형</dt>
              <dd>{selectedTaskTypeLabel}</dd>
            </div>
            <div className="flex justify-between gap-4 border-b-[2px] border-dashed border-foreground/20 pb-2">
              <dt className="font-bold text-foreground">결과 포맷</dt>
              <dd>{selectedOutputLabels.join(", ")}</dd>
            </div>
            <div className="flex justify-between gap-4 border-b-[2px] border-dashed border-foreground/20 pb-2">
              <dt className="font-bold text-foreground">실행 모드</dt>
              <dd>{executionMode === "parallel" ? "레이어별 병렬 실행" : "순차 실행"}</dd>
            </div>
            <div className="flex justify-between gap-4 border-b-[2px] border-dashed border-foreground/20 pb-2">
              <dt className="font-bold text-foreground">URL</dt>
              <dd className="max-w-[70%] truncate text-right">{normalizedUrl || "없음"}</dd>
            </div>
            <div className="flex justify-between gap-4 border-b-[2px] border-dashed border-foreground/20 pb-2">
              <dt className="font-bold text-foreground">텍스트 길이</dt>
              <dd>{sourceText.trim().length > 0 ? `${sourceText.trim().length}자` : "없음"}</dd>
            </div>
            <div className="flex justify-between gap-4 border-b-[2px] border-dashed border-foreground/20 pb-2">
              <dt className="font-bold text-foreground">파일</dt>
              <dd>{uploadedFile ? `${uploadedFile.name} (${formatFileSize(uploadedFile.size)})` : "없음"}</dd>
            </div>
            <div className="flex justify-between gap-4 border-b-[2px] border-dashed border-foreground/20 pb-2">
              <dt className="font-bold text-foreground">세부 요청사항</dt>
