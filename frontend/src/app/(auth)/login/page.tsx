...
export default function LoginPage() {
  ...
  const onSubmit = async (data: LoginFormData) => {
    ...
    try {
      ...
      setLoginError(err.message || '로그인에 실패했습니다. 다시 시도해주세요.');
    } finally {
      setIsLoading(false);
    }
  };
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          ...
          {(loginError || error) && (
            <div
              ...
              role="alert"
            >
              <p className="text-sm font-bold text-red-600">{loginError || error}</p>
            </div>
          )}
          ...
          <div className="relative">
            ...
            <div className="relative flex justify-center text-xs uppercase">
              <span className="bg-white px-2 font-bold text-muted-foreground">
                또는
              </span>
            </div>
          </div>
          ...
          <Button
            type="button"
            variant="outline"
            className="w-full border-[2px] border-foreground font-bold shadow-[4px_4px_0_0_rgba(0,0,0,1)] hover:shadow-[2px_2px_0_0_rgba(0,0,0,1)] hover:translate-x-[2px] hover:translate-y-[2px] transition-all"
            disabled={true}
          >
            <svg
